...
export const selectData = (group, allTickets, orderValue) => async (dispatch) => {
  try {
    ...
    if (group === 'status') {
      allTickets.forEach((elem) => mySet.add(elem.status));
      arr = [...mySet];
      arr.forEach((elem, index) => {
        let arr = allTickets.filter((fElem) => elem === fElem.status);
        selectedData.push({
          [index]: {
            title: elem,
            value: arr
      ...
      allTickets.allUser.forEach((elem, index) => {
        arr = allTickets.allTickets.filter((Felem) => elem.id === Felem.userId);
        selectedData.push({
          [index]: {
            title: elem.name,
            value: arr
      ...
      prior_list.forEach((elem, index) => {
        arr = allTickets.filter((fElem) => index === fElem.priority);
        selectedData.push({
          [index]: {
            title: elem,
            value: arr
    ...
    if (orderValue === "title") {
      selectedData.forEach((elem) => {
        Object.values(elem).forEach((e) => e.value.sort((a, b) => a.title.localeCompare(b.title)));
      });
    }

    if (orderValue === "priority") {
      selectedData.forEach((elem) => {
        Object.values(elem).forEach((e) => e.value.sort((a, b) => b.priority - a.priority));
      });
    }

    dispatch({ type: 'SELECT_DATA_SUCCESS', payload: { selectedData, user } });
  } catch (error) {
    dispatch({ type: 'SELECT_DATA_FAILURE', payload: { message: error.message } });
  }
};
